...
/*   Updated: 2019/06/12 20:05:37 by ksharlen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static long long int			ft_signed_format(va_list format)
{
	...
	ret = 0;
	if (!(ft_strcmp(g_spec.size_spec, "l")))
		ret = va_arg(format, long int);
	else if (!(ft_strcmp(g_spec.size_spec, "ll")))
		ret = va_arg(format, long long int);
	else if (!(ft_strcmp(g_spec.size_spec, "h")))
		ret = (short)va_arg(format, int);
	else if (!(ft_strcmp(g_spec.size_spec, "hh")))
		ret = (char)va_arg(format, int);
	else if (!(ft_strcmp(g_spec.size_spec, "j")))
		ret = va_arg(format, intmax_t);
	else if (!(ft_strcmp(g_spec.size_spec, "z")))
		ret = va_arg(format, ssize_t);
	else if ((!g_spec.size_spec))
		ret = va_arg(format, int);
	return (ret);
}

static unsigned long long int	ft_signed_format(va_list format)
{
